...
PUBLIC int do_noquantum(message *m_ptr)
{
	...
	int rv, proc_nr_n;
	register struct schedproc *rmp1;
	...
	int i;
	for (i=1; i<=proc_nr_n; i++){
		int j = 0;
		for (j=0; j<i; j++){
			if ((&schedproc[i])->procgrp == (&schedproc[j])->procgrp){
				break;
			}
		}
		if (i == j){
	...
	rmp->proc_usage = (rmp->proc_usage)/2;
	rmp->grp_usage = (rmp->grp_usage)/2;
	rmp->fss_priority = (rmp->proc_usage)/2 + ((rmp->grp_usage)*number_of_groups)/4 + 0;
	for (i=0; i<=proc_nr_n; i++){
		rmp1 = &schedproc[i];
		if (rmp1->procgrp == rmp->procgrp){
			rmp1->grp_usage = rmp->grp_usage;
		}
	}
