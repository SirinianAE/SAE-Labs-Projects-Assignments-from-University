...
#define PUT_MODE           2
#define USER_MODE          3
#define PUT_AND_GET_MODE   4
#define NUMBER_OF_FORKS	   1
...
void print_usage() {
  ...
  fprintf(stderr, "-x:             Repeatedly send PUT and GET operations.\n");
}
...
int main(int argc, char **argv) {
  ...
  int pids[NUMBER_OF_FORKS];
  bool parent_flag = true;
  int number_of_childs = 0;

  // Parse user parameters.
  while ((option = getopt(argc, argv,"i:hgpxo:a:")) != -1) {
    switch (option) {
      case 'h':
        print_usage();
        exit(0);
	...
	break;
      case 'g':
        if (mode) {
          fprintf(stderr, "You can only specify one of the following: -g, -p, -o, -x\n");
          exit(EXIT_FAILURE);
        }
        mode = GET_MODE;
        break;
      ...
      case 'x':
        ...
        mode = PUT_AND_GET_MODE;
        break;
      case 'o':
        if (mode) {
          fprintf(stderr, "You can only specify one of the following: -r, -w, -o, -x\n");
          exit(EXIT_FAILURE);
        }
        mode = USER_MODE;
        request = optarg;
  ...
  // Check parameters.
  if (!mode) {
    fprintf(stderr, "Error: One of -g, -p, -o, -x is required.\n\n");
    print_usage();
    exit(0);
  }
  if (!host) {
  ...
  number_of_childs = power(2, NUMBER_OF_FORKS) - 1;
  ...
  } else if (mode == GET_MODE || mode == PUT_MODE){
    while(--count>=0) {
